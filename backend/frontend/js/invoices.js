...
function updatePreview() {
  ...
  if (dueDateInput?.value) {
    ...
    if (previewDueDate) previewDueDate.textContent = formatted;
    if (previewDueIn) {
      const daysLeft = Math.ceil((date - new Date()) / (1000*60*60*24));
      previewDueIn.textContent = daysLeft > 0 ? `${daysLeft} days` : 'Overdue';
    }
  }
  ...
  if (summaryDue) summaryDue.textContent = previewDueDate?.textContent || '';
  if (summaryClient) {
      const email = clientEmailInput?.value?.trim();
...
async function publishInvoice() {
  try {
    ...
    if (shareLinkEl) shareLinkEl.textContent = paylaLink;
